...
activations = ['relu', 'linear', 'sigmoid', 'tanh', 'silu']
...
for idx, model_name in enumerate(tqdm(model_name_list)):
...
def plot_cut_point(ax, cut_point_list, y_len):
    left, S_left = cut_point_list[0]
    right, S_right = cut_point_list[1]
    
    top, down = min(S_left, S_right) - 0.2, min(S_left, S_right) - 3
    
    ax.plot([left, left], [down, top], color=colors[1], ls=':')
    ax.plot([right, right], [down, top], color=colors[1], ls=':')

    pos = down + (top - down) * 0.5
    arrowstyle = 'fancy'
    ax.annotate('', xy=(right, pos), xytext=(left, pos),
                arrowprops=dict(arrowstyle=arrowstyle, color=colors[1], lw=1))
...
# Direct embedding
print('Plot direct embedding')
variant = ['DIS', 'Linear', 'Sigmoid', 'Tanh', 'SiLU', 'ReLU']

cut_point_list = []
y_list = []
for idx, emb in enumerate(emb_list):
    model_name = printed_name[idx]
    num_field, row, col = get_num_field(dataset_name)
    if 'DCNv2' in model_name:
        y = emb['X_0'].flatten(1)
    elif 'CrossNet' in model_name:
        y = emb['X_0'].flatten(1)
    else:
