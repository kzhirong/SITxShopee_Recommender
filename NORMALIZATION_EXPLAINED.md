...
Creates the same 23 columns for ALL datasets:
...
# Total: 1 (label) + 21 (features) + 1 (hour) = 23 columns
```
...
# NOTE: x1 is missing feat_21 → padded with constant -1
```
...
app_id → feat_6
...
C21 → feat_21           # x2 HAS feat_21 (unlike x1)
...
banner_pos → feat_2
...
C21 → feat_21        # x4 HAS feat_21 (unlike x1)
hour → hour          # Already correct 8-digit YYMMDDHH format!
...
- hour = **142416** (6-digit format: YYDDHH for Oct 24, hour 16)

**⚠️ IMPORTANT BUG**: X2's reconstructed hour is in **6-digit format** (YYDDHH) like `142416` instead of 8-digit (YYMMDDHH) like `14102416`. This is a known issue in the normalization script. The code at line 174 should produce `14102416` but actually produces `142416` due to the formatting logic. FuxiCTR's preprocessing can handle both formats by extracting temporal components.
...
├── train.csv    (28.3M rows, 23 columns)
├── valid.csv    (4.0M rows, 23 columns)
└── test.csv     (8.1M rows, 23 columns)

data/Avazu/avazu_x2_normalized/
├── train.csv    (32.3M rows, 23 columns)
└── test.csv     (8.1M rows, 23 columns)

data/Avazu/avazu_x4_normalized/
├── train.csv    (28.1M rows, 23 columns)
├── valid.csv    (4.0M rows, 23 columns)
└── test.csv     (4.0M rows, 23 columns)
```
...
### Output (x2_normalized):
```csv
label,feat_1,feat_2,feat_3,...,feat_21,hour
0,1005,0,1fbe01fe,...,<value>,142416
# Note: hour is in 6-digit format (YYDDHH) for x2 due to bug
# x1 and x4 use proper 8-digit format (YYMMDDHH)
```
...
└────────────────┬────────────────────────────────────────────┘
                 ...
                 ├─> Schema unification (23 columns)
                 │
                 ▼
┌─────────────────────────────────────────────────────────────┐
│              NORMALIZED DATASETS                            │
├─────────────────────────────────────────────────────────────┤
│  x1_normalized: 23 cols (label, feat_1-21, hour)           │
│  x2_normalized: 23 cols (label, feat_1-21, hour)*          │
│  x4_normalized: 23 cols (label, feat_1-21, hour)           │
│  * Note: x2 has 6-digit hour format (bug)                  │
│  ✅ Same schema  ⚠️ x2 hour format differs  ✅ Training OK │
└─────────────────────────────────────────────────────────────┘
```
...
2. **Creates unified schema** - All datasets → same 23 columns (label + 21 features + hour)
...
6. **Known issue**: x2 has 6-digit hour format (bug) but FuxiCTR handles it correctly
