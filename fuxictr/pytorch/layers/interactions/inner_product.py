...
class InnerProductInteraction(nn.Module):
    ...
    def forward(self, feature_emb, left_emb=None):
        if left_emb is None:
            left_emb = feature_emb
        if self._output_type in ["product_sum", "bi_interaction"]:
            ...
            if self.analyzing and self.training:
                bi_interaction.retain_grad()
            self.grad_var_1 = bi_interaction
            ...
            triu_values = triu_values.view(-1, self.interaction_units)
            return triu_values
