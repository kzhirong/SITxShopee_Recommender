...
class FeatureEmbedding(nn.Module):
    ...
    def set_structure(self, structure_list, mode='mean'):
        self.embedding_layer.set_structure(structure_list, mode=mode)
...
class FeatureEmbeddingDict(nn.Module):
    def __init__(self, 
        ...
        self.structure_list = [[idx] for idx in range(len(self.embedding_layers))]
    ...
    def emb_aggregation(self, feature_emb_list):
        out_emb_list = []
        for source_list in self.structure_list:
            out_emb_list.append(torch.stack([feature_emb_list[idx] for idx in source_list], dim=1).mean(dim=1))
        return out_emb_list
    ...
    def dict2tensor(self, embedding_dict, flatten_emb=False, feature_list=[], feature_source=[],
        ...
        feature_emb_list = self.emb_aggregation(feature_emb_list)
        if flatten_emb:
