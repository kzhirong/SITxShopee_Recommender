#!/usr/bin/env python3
"""
LLM-CTR Single-Phase Training Pipeline for Avazu Dataset (Qwen3-1.7B)

This script implements single-phase training on x4 only using Qwen3-1.7B:
- Train Feature Encoder + GEN + Projector + LLM together on x4
- Integrated evaluation on x4 test set after training

Architecture:
    Feature IDs → Embeddings → Encoder (GEN) → Projector → [Text + Features] → LLM (1.7B) → Token Logits

Training:
    - Trained: Feature Encoder + Encoder (GEN) + Projector + LLM (all trained together from scratch)
    - Model: Qwen3-1.7B (hidden_size=2048)

Usage:
    python train_llm_single_phase_1.7B.py --epochs 1 --batch_size 128 --gpu 0
"""
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=128,
                    help='Batch size (default: 128, reduced for 1.7B model memory)')
    ...
    print("\n" + "-" * 80)
    print("STEP 2: Loading LLM (Qwen3-1.7B)")
    print("-" * 80)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_auc > best_val_auc:
            ...
            save_dir = Path('checkpoints/llm_single_phase_1.7B')
